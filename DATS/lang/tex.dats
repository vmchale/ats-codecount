...
staload "SATS/lang/tex.sats"
staload "SATS/lang/common.sats"

implement free_st_tex (st) =
  case+ st of
    | ~line_comment() => ()
    | ~regular() => ()
    | ~post_newline_whitespace() => ()

implement parse_state_tex_tostring (st) =
  case+ st of
    | regular() => "regular"
    | line_comment() => "line_comment"
    | post_newline_whitespace() => "post_newline_whitespace"

implement free$lang<parse_state_tex> (st) =
  free_st_tex(st)

implement init$lang<parse_state_tex> (st) =
  st := post_newline_whitespace

implement advance_char$lang<parse_state_tex> (c, st, file_st) =
  case+ st of
    | regular() =>
      begin
        case+ c of
